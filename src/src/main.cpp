 ...
 *
 * $Revision: 9733 $
 * $Id: main.cpp 9733 2014-03-12 19:46:38Z alpha0010 $
...
const static wxString gDefaultLayout = _T("Novice");
...
///*4 lines added Ron*/
int idFileSamplePrograms                = XRCID("idSamplePrograms");
int idFileSampleProgramsC               = XRCID("idSampleProgramsC");
int idFileSampleProgramsCpp             = XRCID("idSampleProgramsCpp");

int idSampleOne				= XRCID("idSampleOne");
int idSampleTwo				= XRCID("idSampleTwo");
int idSampleThree			= XRCID("idSampleThree");
int idSampleFour			= XRCID("idSampleFour");
int idSampleFive			= XRCID("idSampleFive");
int idSampleSix				= XRCID("idSampleSix");
int idSampleSeven			= XRCID("idSampleSeven");
int idSampleEight			= XRCID("idSampleEight");
int idSampleNine			= XRCID("idSampleNine");
int idSampleTen				= XRCID("idSampleTen");

int idSampleOneC				= XRCID("idSampleOneC");
int idSampleTwoC				= XRCID("idSampleTwoC");
int idSampleThreeC				= XRCID("idSampleThreeC");
int idSampleFourC				= XRCID("idSampleFourC");
int idSampleFiveC				= XRCID("idSampleFiveC");
int idSampleSixC				= XRCID("idSampleSixC");
int idSampleSevenC				= XRCID("idSampleSevenC");
int idSampleEightC				= XRCID("idSampleEightC");
int idSampleNineC				= XRCID("idSampleNineC");
int idSampleTenC				= XRCID("idSampleTenC");
//Edit ended
...
BEGIN_EVENT_TABLE(MainFrame, wxFrame)
    ...
    //Edited By Ron
    EVT_MENU(idSampleOne ,   MainFrame::OnFileSampleCWhat)
	EVT_MENU(idSampleTwo ,   MainFrame::OnFileSampleCWhat)
	EVT_MENU(idSampleThree ,   MainFrame::OnFileSampleCWhat)
	EVT_MENU(idSampleFour ,   MainFrame::OnFileSampleCWhat)
	EVT_MENU(idSampleFive ,   MainFrame::OnFileSampleCWhat)
	EVT_MENU(idSampleSix ,   MainFrame::OnFileSampleCWhat)
	EVT_MENU(idSampleSeven ,   MainFrame::OnFileSampleCWhat)
	EVT_MENU(idSampleEight ,   MainFrame::OnFileSampleCWhat)
	EVT_MENU(idSampleNine ,   MainFrame::OnFileSampleCWhat)
	EVT_MENU(idSampleTen ,   MainFrame::OnFileSampleCWhat)

	EVT_MENU(idSampleOneC ,   MainFrame::OnFileSampleCppWhat)
	EVT_MENU(idSampleTwoC ,   MainFrame::OnFileSampleCppWhat)
	EVT_MENU(idSampleThreeC ,   MainFrame::OnFileSampleCppWhat)
	EVT_MENU(idSampleFourC ,   MainFrame::OnFileSampleCppWhat)
	EVT_MENU(idSampleFiveC ,   MainFrame::OnFileSampleCppWhat)
	EVT_MENU(idSampleSixC ,   MainFrame::OnFileSampleCppWhat)
	EVT_MENU(idSampleSevenC ,   MainFrame::OnFileSampleCppWhat)
	EVT_MENU(idSampleEightC ,   MainFrame::OnFileSampleCppWhat)
	EVT_MENU(idSampleNineC ,   MainFrame::OnFileSampleCppWhat)
	EVT_MENU(idSampleTenC ,   MainFrame::OnFileSampleCppWhat)
...
void MainFrame::CreateIDE()
{
    ...
    //added
    // opens initial editor
    cbEditor* ed = Manager::Get()->GetEditorManager()->New();
    wxString code = _T("#include<simplecpp>\n\nmain_program{\n//Write your code here\n\n}");
...
void MainFrame::OnFileSampleCWhat(wxCommandEvent& event)
{
	...
	if(id == idSampleOne )
	{
        	tipsFile = ConfigManager::GetDataFolder() + _T("/hello.c");
   	}
	else
	if(id == idSampleTwo )
	{
	...
	if(id == idSampleTen )
	{
	    ...
	    cbEditor* ed = Manager::Get()->GetEditorManager()->Open(tipsFile);
	    if (ed)
	    {
		// Cryogen 24/3/10 Activate the editor after opening. Partial fix for bug #14087.
		ed->Activate();
...
void MainFrame::OnFileSampleCppWhat(wxCommandEvent& event)
{
	wxString tipsFile;
	int id = event.GetId();
	if(id == idSampleOneC )
	...
	if(id == idSampleSevenC )
	{
        	tipsFile = ConfigManager::GetDataFolder() + _T("/hello.c");
   	}
	else
	if(id == idSampleEightC )
	{
        	tipsFile = ConfigManager::GetDataFolder() + _T("/hello.c");
   	}
	else
...
void MainFrame::OnFileNew(cb_unused wxCommandEvent& event)
{
    ...
    bar->FindItem(idFileNewEmpty, &popup);
    if (popup)
    {
...
void MainFrame::OnViewLayoutDelete(cb_unused wxCommandEvent& event)
{
    ...
    {
        if (cbMessageBox(_("The Code::Blocks default perspective cannot be deleted. It can always be reverted to "
